...
public enum Flags {
    /***********************************************************
    @brief Skip repeated messages, this requires the user app to use LibAVUtil.Log.Class.av_log () instead of
    (f)printf as the 2 would otherwise interfere and lead to
    "Last message repeated x times" messages below (f)printf messages with some
    bad luck.
    Also to receive the last, "last repeated" line if any, the user app must
    call LibAVUtil.Log.Class.av_log (null, LibAVUtil.Log.Level.QUIET, "%s", ""); at the end
    ***********************************************************/
    SKIP_REPEATED,
