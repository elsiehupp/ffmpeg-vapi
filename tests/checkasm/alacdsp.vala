...
private static void check_decorrelate_stereo () {
    //  LOCAL_ALIGNED_16 (int32, ref_buf, [BUF_SIZE*MAX_CHANNELS]);
    //  LOCAL_ALIGNED_16 (int32, new_buf, [BUF_SIZE*MAX_CHANNELS]);
    ...
    if (check_func (c.decorrelate_stereo, "alac_decorrelate_stereo")) {
        ...
        int shift =  rnd () & 0x1F;
        int weight =  rnd () & 0xFF;
        //  declare_func (void, int32[] buf[2], int len, int shift, int weight);

        randomize_buffers ();
        //  call_ref (ref, len, shift, weight);
        //  call_new (new, len, shift, weight);
        //  if (memcmp (ref[0], new[0], len * sizeof (int32)) ||
        //      memcmp (ref[1], new[1], len * sizeof (int32))) {
        //      fail ();
        //  }
...
private static void check_append_extra_bits () {
    //  LOCAL_ALIGNED_16 (int32, ref_buf, [BUF_SIZE*MAX_CHANNELS*2]);
    //  LOCAL_ALIGNED_16 (int32, new_buf, [BUF_SIZE*MAX_CHANNELS*2]);
    int32[] ref[2] = { &ref_buf[BUF_SIZE*0], &ref_buf[BUF_SIZE*1] };
    int32[] new[2] = { &new_buf[BUF_SIZE*0], &new_buf[BUF_SIZE*1] };
    ...
    for (ch = 1; ch <= 2; ch++) {
        if (check_func (c.append_extra_bits[ch-1], "alac_append_extra_bits_%s", channels[ch-1])) {
            int len = (rnd () & 0xFF) + 1;
            //  declare_func (void, int32[] buf[2], int32[] ebb[2], int ebits, int ch, int len);

            randomize_buffers ();
            //  call_ref (ref, ref_ebb, 8, ch, len);
            //  call_new (new, new_ebb, 8, ch, len);
            //  if (            memcmp (ref[0], new[0], len * sizeof (int32)) ||
            //      (ch == 2 && memcmp (ref[1], new[1], len * sizeof (int32)))) {
            //      fail ();
            //  }
