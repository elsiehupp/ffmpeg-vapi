...
int float_near_ulp (float a, float b, uint max_ulp) {
    ...
    if (llabs ((int64)x.i - y.i) <= max_ulp)
        return 1;
...
int float_near_abs_eps_array_ulp (
    float[] a,
    float[] b,
    float eps,
    uint max_ulp,
...
static int measure_nop_time () {
    ...
    qsort (nops, 10000, sizeof (uint16), cmp_nop);
    for (i = 2500; i < 7500; i++)
        nop_sum += nops[i];
