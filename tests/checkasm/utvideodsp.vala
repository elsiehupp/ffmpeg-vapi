...
padded to 32
***********************************************************/
const size_t WIDTH_PADDED = (WIDTH + 16);

const size_t BUFFER_SIZE = (WIDTH_PADDED * HEIGHT);


void randomize_plane (void *buf, type) {
    int w, h;
    type * tmp = buf;
    for (h = 0; h < HEIGHT; h++) {
        for (w = 0; w < WIDTH; w++)
