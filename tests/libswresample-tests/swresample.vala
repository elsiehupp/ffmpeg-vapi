...
private const size_t SAMPLES = 1000;

private const size_t SWR_CH_MAX = 32;

private const int ASSERT_LEVEL = 2;

private static double get (uint8[] a[], int ch, int index, int ch_count, LibAVUtil.SampleFormat f){
    uint8[] p;
    ...
    switch (f){
    case LibAVUtil.SampleFormat.UNSIGNED_8_BIT : return ((uint8*)p)[index]/127.0-1.0;
    case LibAVUtil.SampleFormat.SIGNED_16_BIT: return ((int16*)p)[index]/32767.0;
    case LibAVUtil.SampleFormat.SIGNED_32_BIT: return ((int32[])p)[index]/2147483647.0;
    case LibAVUtil.SampleFormat.FLOAT: return ((float[])p)[index];
    case LibAVUtil.SampleFormat.DOUBLE: return ((double[] )p)[index];
    default: av_assert0 (0);
    }

}

private static void  set (uint8[] a[], int ch, int index, int ch_count, LibAVUtil.SampleFormat f, double v){
    uint8[] p;
    if (av_sample_fmt_is_planar (f)){
        f= av_get_alt_sample_fmt (f, 0);
        p= a[ch];
    ...
    switch (f){
    case LibAVUtil.SampleFormat.UNSIGNED_8_BIT : ((uint8*)p)[index]= av_clip_uint8 (lrint ((v+1.0)*127));     break;
    case LibAVUtil.SampleFormat.SIGNED_16_BIT: ((int16*)p)[index]= av_clip_int16 (lrint (v*32767));         break;
    case LibAVUtil.SampleFormat.SIGNED_32_BIT: ((int32*)p)[index]= av_clipl_int32 (llrint (v*2147483647));   break;
    case LibAVUtil.SampleFormat.FLOAT: ((float  *)p)[index]= v;                                      break;
    case LibAVUtil.SampleFormat.DOUBLE: ((double[] )p)[index]= v;                                      break;
    default: av_assert2 (0);
    }

}

private static void shift (uint8[] a[], int index, int ch_count, LibAVUtil.SampleFormat f){
    int ch;
...
private static void setup_array (uint8[] out[SWR_CH_MAX], uint8[] in, LibAVUtil.SampleFormat format, int samples){
...
private static int cmp (void *a, void *b){
    return *(int[] )a - *(int[] )b;
}
...
private static void PUT_SAMPLE () {
    set (data, ch, k, channels, sample_fmt, v);
}

private static void uint_rand (
    ref uint x
) {
    ((x) = (x) * 1664525 + 1013904223);
}

private static void dbl_rand (
    ref double x
) {
...
private static int main (
    ...
    uint64 in_ch_layout, out_ch_layout;
    LibAVUtil.SampleFormat in_sample_fmt, out_sample_fmt;
    uint8 array_in[SAMPLES*8*8];
    uint8 array_mid[SAMPLES*8*8*3];
    uint8 array_out[SAMPLES*8*8+100];
    uint8[] ain[SWR_CH_MAX];
    uint8[] aout[SWR_CH_MAX];
    uint8[] amid[SWR_CH_MAX];
    int flush_i=0;
    int mode;
    int num_tests = 10000;
    uint32 seed = 0;
    ...
    SwrContext? forw_ctx= null;
    SwrContext? backw_ctx= null;
    ...
    for (test=0; test<num_tests; test++){
        uint r;
        uint_rand (seed);
        r = (seed * (uint64)(max_tests - test)) >>32;
    ...
    for (test=0; test<num_tests; test++){
        ...
        uint vector= remaining_tests[max_tests - test - 1];
        int in_ch_count;
        ...
        if (mode==0 /*|| out_sample_rate == in_sample_rate*/) {
            mid_count= swr_convert (forw_ctx, amid, 3*SAMPLES, (uint8[][] )ain, SAMPLES);
        } else if (mode==1){
            mid_count= swr_convert (forw_ctx, amid, 0, (uint8[][] )ain, SAMPLES);
            mid_count+=swr_convert (forw_ctx, amid, 3*SAMPLES, (uint8[][] )ain, 0);
        } else {
            int tmp_count;
            mid_count= swr_convert (forw_ctx, amid, 0, (uint8[][] )ain, 1);
        ...
        out_count= swr_convert (backw_ctx,aout, SAMPLES, (uint8[][] )amid, mid_count);
