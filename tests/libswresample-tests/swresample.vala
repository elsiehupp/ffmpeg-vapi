...
public static double get (uint8[] a[], int ch, int index, int ch_count, enum AVSampleFormat f){
    const uint8[] p;
...
public static void  set (uint8[] a[], int ch, int index, int ch_count, enum AVSampleFormat f, double v){
    uint8[] p;
    if (av_sample_fmt_is_planar (f)){
        f= av_get_alt_sample_fmt (f, 0);
        p= a[ch];
...
public static void shift (uint8[] a[], int index, int ch_count, enum AVSampleFormat f){
    int ch;
...
public static const enum AVSampleFormat formats[] = {
    AV_SAMPLE_FMT_S16,
    AV_SAMPLE_FMT_FLTP,
    AV_SAMPLE_FMT_S16P,
    AV_SAMPLE_FMT_FLT,
...
public static const uint64_t layouts[]={
    AV_CH_LAYOUT_MONO                    ,
    AV_CH_LAYOUT_STEREO                  ,
    AV_CH_LAYOUT_2_1                     ,
...
public static void setup_array (uint8[] out[SWR_CH_MAX], uint8[] in, enum AVSampleFormat format, int samples){
...
public static void audiogen (void *data, enum AVSampleFormat sample_fmt,
...
public static int main (
    ...
    for (test=0; test<num_tests; test++){
        ...
        if (mode==0 /*|| out_sample_rate == in_sample_rate*/) {
            mid_count= swr_convert (forw_ctx, amid, 3*SAMPLES, (const uint8[] *)ain, SAMPLES);
        } else if (mode==1){
            mid_count= swr_convert (forw_ctx, amid,         0, (const uint8[] *)ain, SAMPLES);
            mid_count+=swr_convert (forw_ctx, amid, 3*SAMPLES, (const uint8[] *)ain,       0);
        } else {
            int tmp_count;
            mid_count= swr_convert (forw_ctx, amid,         0, (const uint8[] *)ain,       1);
