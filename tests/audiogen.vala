...
public class audiogen {
    ...
    ***********************************************************/
    ...
    static void put_wav_header (
        ...
        uint block_align = SAMPLE_SIZE * channels;
        uint data_size = block_align * nb_samples;
    ...
    uint main (
        ...
        uint ampa;
        uint seed = 1;
        uint tabf1[MAX_CHANNELS];
        uint tabf2[MAX_CHANNELS];
        uint taba[MAX_CHANNELS];
        uint sample_rate = 44100;
        uint nb_channels = 2;
        char[] ext;

        if (argc < 2 || argc > 5) {
            GLib.print ("usage: %s file [<sample rate> [<channels>] [<random seed>]]\n" +
    ...
    ***********************************************************/
        ...
        for (uint i = 0; i < sample_rate / 2; i++) {
            v = myrnd (out seed, 20000) - 10000;
            for (uint j = 0; j < nb_channels; j++)
                put16 (v);
        }
    ...
    ***********************************************************/
        ...
        a = 0;
        ampa = 0;
        for (uint i = 0; i < 2 * sample_rate; i++) {
            for (uint j = 0; j < nb_channels; j++) {
